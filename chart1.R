...
ui <- fluidPage(
  titlePanel("Trends of stock prices of Tesla and Vollkswagen"),
  sidebarLayout(
    sidebarPanel(
      h5("Controls"),
      sliderInput(
...
server <- function(input, output){
  output$line <- renderPlot({
    ggplot(data=filtered_date, aes(x=Date, group = 1))+
      geom_line(aes(y=avg_price_tesla))+
      geom_line(aes(y=avg_price_vollkswagen))
